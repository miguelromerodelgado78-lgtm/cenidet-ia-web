...
import { 
  ...
  ChevronRight, Bookmark, ShieldCheck, Users // Agregado Users por seguridad
} from 'lucide-react';
...
const Home = () => {
  ...
  const [loading, setLoading] = useState(true); // Estado de carga para evitar bloqueos
  ...
  // Si está cargando, mostramos un mensaje simple para evitar que la pantalla se quede en blanco
  if (loading) {
    return (
  ...
  <button className="bg-[#49a5e6] hover:bg-white text-[#072146] px-10 py-4 rounded-sm font-bold text-xs uppercase tracking-widest transition-all shadow-lg">
    ...
    {/* --- SECCIÓN 2: COMUNICADOS (GRUPO Y DOCTORES) --- */}
{featuredNews.length > 0 && (
  <section id="noticias" className="py-24 px-6 max-w-7xl mx-auto space-y-20">
      ...
      return (
        <div className="animate-in fade-in slide-in-from-bottom duration-700">
          ...
          <div className="relative group/container">
            {/* SOLO MOSTRAR BOTONES SI HAY MÁS DE 1 */}
            {groupNews.length > 1 && (
              <>
                ...
                <div key={item.id} className="flex-none w-[90vw] md:w-[800px] snap-center bg-[#072146] text-white rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row border border-white/10">
                  {item.image && (
                    <div className="md:w-96 h-64 md:h-auto overflow-hidden shrink-0">
                      <img src={item.image.startsWith('http') ? item.image : `http://127.0.0.1:8000${item.image}`} alt="Oficial" className="w-full h-full object-cover opacity-80" />
                    </div>
                  )}
    ...
    {/* --- 2.2 ACTIVIDAD DE INVESTIGADORES (Noticias Destacadas) --- */}
    {(() => {
                ...
                <div key={item.id} className="flex-none w-[280px] md:w-[400px] snap-start bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-xl transition-all flex flex-col overflow-hidden group">
                  {item.image && (
                    <div className="w-full h-44 overflow-hidden">
                      <img src={item.image.startsWith('http') ? item.image : `http://127.0.0.1:8000${item.image}`} alt="News" className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                    </div>
                  )}
...
<section 
  ...
  style={{
    // Reemplaza 'URL_DE_TU_IMAGEN' por la ruta (ej. /img/campus.jpg o una importación)
    backgroundImage: `linear-gradient(rgba(7, 33, 70, 0.85), rgba(7, 33, 70, 0.85)), url('/fondo3.jpeg')`,
    backgroundSize: 'cover',
    backgroundPosition: 'center',
    backgroundAttachment: 'fixed' // Esto le da un efecto elegante al hacer scroll
  }}
>
  <div className="max-w-7xl mx-auto relative z-10">
   ...
   {/* --- SECCIÓN 4: PROYECTOS (Solo se muestra si hay proyectos) --- */}
{projects.length > 0 && (
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {projects.map(project => (
        ...
        >
          ...
          {/* Sustituye el botón dentro del mapeo de proyectos */}
